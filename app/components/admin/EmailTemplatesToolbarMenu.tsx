...
const menuItemClass =
  "flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-gray-800 hover:bg-gray-100 dark:text-gray-100 dark:hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-50";
...
export function EmailTemplatesToolbarMenu({
  ...
  return (
    <div
      ref={dropdownRef}
      className="absolute right-0 top-full z-50 mt-1 min-w-[12.5rem] rounded-lg border border-gray-200 bg-white py-1 shadow-lg dark:border-slate-600 dark:bg-slate-800"
      role="menu"
    >
      <button
        type="button"
      ...
      >
        <Upload
          ...
          aria-hidden
        />
        {importBusy ? "Importing…" : "Import templates"}
      </button>
    </div>
  );
}
