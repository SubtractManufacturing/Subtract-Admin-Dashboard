...
export async function action({ request }: ActionFunctionArgs) {
  ...
  if (request.headers.get("content-type")?.includes("multipart/form-data")) {
        ...
        if (!isConversionEnabled()) {
          ...
          return withAuthHeaders(
            json({
              ...
              cadUrl: cadKey,
              message:
                "CAD file uploaded, but conversion service is not available",
            }),
            headers,
          );
        }
      ...
      try {
        ...
        return withAuthHeaders(
          json({
            ...
            meshUrl: meshKey,
            message:
              "Placeholder CAD and mesh saved for drawing-only quote parts",
          }),
          headers,
        );
      } catch (error) {
    ...
    if (
      ...
      if (
        !file ||
        !entityType ||
        !["customers", "vendors"].includes(entityType)
      ) {
        return withAuthHeaders(
          json(
            {
              error: "File and entity type (customers or vendors) are required",
            },
            { status: 400 },
          ),
          headers,
      ...
      try {
        ...
        const { parseImportFile, getImportPreview } =
          await import("~/lib/bulk-import");
        const buffer = Buffer.from(await file.arrayBuffer());
        const { rows } = parseImportFile(buffer);
        const preview = await getImportPreview(
          rows,
          entityType as "customers" | "vendors",
        );
  ...
  if (
    ...
    if (
      ...
      return withAuthHeaders(
        json(
          { error: "entityType, rows, and actions are required" },
          { status: 400 },
        ),
        headers,
      );
    ...
    try {
      ...
      const { validateCustomerRow, validateVendorRow } =
        await import("~/lib/bulk-import");
      ...
      const validate =
        entityType === "customers" ? validateCustomerRow : validateVendorRow;

      for (const row of preview) {
        const actionKey = String(row.rowIndex);
        const action =
          actions[actionKey] ?? (row.match === "new" ? "create" : "skip");
          ...
          try {
            if (entityType === "customers") {
              await createCustomer(
                validated.data as CustomerInput,
                eventContext,
              );
            } else {
        ...
        try {
          ...
          if (entityType === "customers") {
            await updateCustomer(
              row.existingId!,
              updatePayload as Partial<CustomerInput>,
              eventContext,
            );
          } else {
            await updateVendor(
              row.existingId!,
              updatePayload as Partial<VendorInput>,
              eventContext,
            );
          }
          updated++;
...
function PruneResultMessage({ data, state }: { data: unknown; state: string }) {
  ...
  if ("pruneResult" in data) {
    const { total, flags, settings } = (
      data as {
        pruneResult: { total: number; flags: string[]; settings: string[] };
      }
    ).pruneResult;
    if (total === 0) {
      return (
...
export default function Settings() {
  ...
  const [bulkEntity, setBulkEntity] = useState<"customers" | "vendors">(
    "customers",
  );
  ...
  const [importActions, setImportActions] = useState<
    Record<string, "create" | "override" | "modify" | "skip">
  >({});
  ...
  useEffect(() => {
    ...
    if (
      "preview" in data &&
      Array.isArray((data as { preview: BulkImportPreviewRow[] }).preview)
    ) {
      const { preview, entityType } = data as {
        preview: BulkImportPreviewRow[];
        entityType: "customers" | "vendors";
      };
  ...
  useEffect(() => {
    ...
    if (
      importConfirmFetcher.state !== "idle" ||
      !data ||
      typeof data !== "object"
    )
      return;
  ...
  const handleSelectAllExport = () =>
    setSelectedExportIds(bulkList.map((e: Customer | Vendor) => e.id));
                        ...
                        <div className="space-y-3">
                                ...
                                {bulkList.map((item: Customer | Vendor) => (
                                  <li key={item.id}>
                                    <label className="flex items-center gap-2 p-2 sm:p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 cursor-pointer">
                                      <input
                                        type="checkbox"
                                        checked={selectedExportIds.includes(
                                          item.id,
                                        )}
                                        onChange={() =>
                          ...
                          <div className="flex flex-wrap gap-2 items-center">
                            ...
                            <Button
                              ...
                              size="sm"
                              disabled={
                                selectedExportIds.length === 0 || isDownloading
                              }
                              onClick={() => {
                                const format =
                                  (
                            ...
                            >
                              {isDownloading
                                ? "Downloading…"
                                : `Export ${selectedExportIds.length} ${bulkEntity}`}
                            </Button>
                          </div>
                        </div>
                      )}
                          ...
                          {!importPreview ? (
                            <>
                              <p className="text-sm text-gray-600 dark:text-gray-400">
                                Upload a CSV or JSON file to import {bulkEntity}
                                . You can use the template CSV from Export to
                              ...
                              >
                                ...
                                {importParseFetcher.data &&
                                  "error" in importParseFetcher.data && (
                                    <p className="text-sm text-red-600 dark:text-red-400">
                                      {
                                        (
                                          importParseFetcher.data as {
                                            error: string;
                                          }
                                        ).error
                                      }
                            ...
                            <div className="space-y-3">
                              <p className="text-sm text-gray-600 dark:text-gray-400">
                                {importPreview.preview.length} row(s) — set
                                action per row, then confirm.
                              </p>
                              ...
                              <div className="flex flex-wrap gap-2">
                                <importConfirmFetcher.Form method="post">
                                  <input
                                    type="hidden"
                                    name="intent"
                                    value="bulkImportConfirm"
                                  />
                                  <input
                                    type="hidden"
                                    name="entityType"
                                    value={importPreview.entityType}
                                  />
                                  <input
                                    type="hidden"
                                    name="rows"
                                  ...
                                  <input
                                    type="hidden"
                                    name="actions"
                                    value={JSON.stringify(importActions)}
                                  />
                                  <Button
                                    type="submit"
                                    variant="primary"
                              ...
                              {importConfirmFetcher.data &&
                                "created" in importConfirmFetcher.data && (
                                  <p className="text-sm text-gray-700 dark:text-gray-300">
                                    Created:{" "}
                                    {
                                      (
                                        importConfirmFetcher.data as {
                                          created: number;
                                        }
                                      ).created
                                    }
                              ...
                              {importConfirmFetcher.data &&
                                "error" in importConfirmFetcher.data && (
                                  <p className="text-sm text-red-600 dark:text-red-400">
                                    {
                                      (
                                        importConfirmFetcher.data as {
            ...
            <div className="max-w-2xl">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                    Advanced features and experimental functionality for
                    developers.
                  </p>
                  <div>
                    {localFeatureFlags
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="flex items-center gap-3">
                    <pruneFetcher.Form method="post">
                      ...
                      >
                        {pruneFetcher.state === "submitting"
                          ? "Pruning..."
                          : "Prune stale data"}
                      </Button>
                    </pruneFetcher.Form>
                    <PruneResultMessage
                      data={pruneFetcher.data}
                      state={pruneFetcher.state}
                    />
                  </div>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                    Removes feature_flags and developer_settings rows that no
